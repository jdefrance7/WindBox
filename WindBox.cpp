...
int initModules()
{
  ...
  // Attempt to Init Anemometer
  if(initAnemometer())
  {
    broadcast("ERROR: Unable to init Anemometer.\n");
    led.error();
  }
  else
  {
    broadcast("INFO: Anemometer enabled...\n");
  }

  if(initCounter())
  {
    broadcast("ERROR: Unable to init Counter.\n");
    led.error();
  }
  else
  {
    broadcast("INFO: Counter enabled...\n");
  }
...
int moduleStatus()
{
  ...
  broadcast("  ANEMOMETER: "+String(ANEMOMETER_ENABLED)+"\n");

  broadcast("  COUNTER: "+String(COUNTER_ENABLED)+"\n");
